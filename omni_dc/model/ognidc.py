...
from omni_dc.depth_models.depth_anything_v2.depth_anything_v2.dpt import DepthAnythingV2
...
class OGNIDC(nn.Module):
    def __init__(self, args, device="cuda"):
        ...
        if self.args.load_dav2:
            ...
            if not hasattr(args, 'da_path'):
                args.da_path = "/home/eongan/ethz/3d_vision/OMNI-DC/checkpoints/depth_anything_v2_vitl.pth"
            self.depth_module = DepthAnythingV2(**dav2_model_configs[encoder])
            self.depth_module.load_state_dict(
                torch.load(args.da_path,
                           map_location='cpu'))
    ...
    def forward(self, sample):
        ...
        B, _, H, W = rgb.shape
        print("HHHHHHHHHHHHHHHHHHHHHHHHHH", H)
        print("HHHHHHHHHHHHHHHHHHHHHHHHHH", W)
        ...
        if self.args.training_depth_random_shift_range > 0.0 and self.training:
            ...
            random_shift = torch.empty(batch_size).uniform_(-0.5,
                                                            0.5).to(self.device) * self.args.training_depth_random_shift_range
