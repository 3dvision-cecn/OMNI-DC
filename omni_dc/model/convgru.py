...
class BasicUpdateBlock(nn.Module):
    ...
    def forward(self, net, inp, depth, depth_grad, K, device="cuda"):
        # net: hidden; inp: ctx
        ...
        if self.args.multi_resolution_learnable_gradients_weights == "learnable":
            ...
            weights_depth_grad = torch.tensor(([1.0] * self.resolution), device=device) / float(self.resolution)
            ...
            weights_depth_grad = torch.tensor(([1.0, 4.0, 16.0]), device=device)[:self.resolution]
        ...
        if self.args.multi_resolution_learnable_input_weights:
            ...
            weights_input = torch.tensor(([1.0] + [0.0] * (self.resolution-1)), device=device)
